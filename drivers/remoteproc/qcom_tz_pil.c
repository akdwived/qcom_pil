...
static int qproc_scm_clk_enable(struct qproc *qproc)
{
	...
	if (qproc->no_scm_clks)
		return 0;
	clk_venus_is_enabled = 5;
	ret = clk_prepare_enable(qproc->scm_core_clk);
	ret1 = __clk_is_enabled(qproc->scm_core_clk);
	if (ret || !ret1)
		goto bail;
	ret = clk_prepare_enable(qproc->scm_iface_clk);
	ret1 = __clk_is_enabled(qproc->scm_iface_clk);
	if (ret || !ret1)
		goto disable_core;
	ret = clk_prepare_enable(qproc->scm_bus_clk);
	ret1 = __clk_is_enabled(qproc->scm_bus_clk);
	if (ret || !ret1)
		goto disable_iface;

	ret = clk_prepare_enable(qproc->scm_src_clk);
	ret1 = __clk_is_enabled(qproc->scm_src_clk);
	if (ret || !ret1)
		goto disable_bus;
    clk_venus_is_enabled = 0;
	return 0;

disable_bus:
	//clk_disable_unprepare(qproc->scm_bus_clk);
	printk("AD: scm_src_clk clock did not turn ON\n");
disable_iface:
	//clk_disable_unprepare(qproc->scm_iface_clk);
	printk("AD: scm_bus_clk clock did not turn ON\n");
disable_core:
	//clk_disable_unprepare(qproc->scm_core_clk);
	printk("AD: scm_iface_clk clock did not turn ON\n");
bail:
	printk("AD: scm_core_clk clock did not turn ON\n");
	clk_venus_is_enabled = 0;
	return ret;
}
				...
				const struct elf32_phdr *phdr, phys_addr_t paddr)
{
	...
	printk(KERN_ALERT"qproc_load_segment addr %x \n", paddr);
...
static int qproc_load(struct rproc *rproc, const struct firmware *fw)
{
	...
	dev_err(&rproc->dev,"PG:qproc_load\n");
	...
	dev_err(&rproc->dev,"AD:qcom_scm_pas_init_image done \n");

	diff_addr = max_addr - min_addr;
	dev_err(qproc->dev, "AD: pas_mem_setup %pa, %pa\n", &min_addr, &diff_addr);
	...
	dev_err(&rproc->dev,"AD:qcom_scm_pas_mem_setup done \n");
	...
	for (i = 0; i < ehdr->e_phnum; i++) {
		...
		printk("Avaneesh qproc_load for segment %d\n", i);
		if (!segment_is_loadable(phdr))
			continue;
	...
	printk("Avaneesh qproc_load succedded\n");
...
static int qproc_start(struct rproc *rproc)
{
	...
	pm_runtime_set_active(qproc->dev);
	pm_runtime_enable(qproc->dev);

	ret = qproc_scm_clk_enable(qproc);
	if (ret)
		goto disable_regulator;

	printk("Avaneesh Calling qcom_scm_pas_auth_and_reset for %d\n", qproc->pas_id);
	...
	if (ret) {
		...
		printk("Avaneesh: auth_and_reset passed \n");
	}
...
static int qproc_init_clocks(struct qproc *qproc)
{
	...
	qproc->scm_core_clk = devm_clk_get(qproc->dev, "core_clk");
	if (IS_ERR(qproc->scm_core_clk)) {
		if (PTR_ERR(qproc->scm_core_clk) != -EPROBE_DEFER)
			dev_err(qproc->dev, "AD: failed to acquire scm_core_clk\n");
		return PTR_ERR(qproc->scm_core_clk);
	}

	qproc->scm_iface_clk = devm_clk_get(qproc->dev, "iface_clk");
	if (IS_ERR(qproc->scm_iface_clk)) {
		if (PTR_ERR(qproc->scm_iface_clk) != -EPROBE_DEFER)
			dev_err(qproc->dev, "AD: failed to acquire scm_iface_clk\n");
		return PTR_ERR(qproc->scm_iface_clk);
	}

	qproc->scm_bus_clk = devm_clk_get(qproc->dev, "bus_clk");
	if (IS_ERR(qproc->scm_bus_clk)) {
		if (PTR_ERR(qproc->scm_bus_clk) != -EPROBE_DEFER)
			dev_err(qproc->dev, "AD: failed to acquire scm_bus_clk\n");
		return PTR_ERR(qproc->scm_bus_clk);
	}

	qproc->scm_src_clk = devm_clk_get(qproc->dev, "maxi_clk");
	if (IS_ERR(qproc->scm_src_clk)) {
		if (PTR_ERR(qproc->scm_src_clk) != -EPROBE_DEFER)
			dev_err(qproc->dev, "AD: failed to acquire scm_src_clk\n");
		return PTR_ERR(qproc->scm_src_clk);
	}

	ret = clk_set_rate(qproc->scm_core_clk,
clk_round_rate(qproc->scm_core_clk, 80000000));
if (ret) {
		dev_err(qproc->dev, "AD: failed to set rate of scm_core_clk\n");
		return ret;
	}
...
rate = clk_round_rate(qproc->scm_src_clk, 19200000);
...
static int qproc_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev,"Avaneesh:venus probe starts\n");
	...
	ret = rproc_add(rproc);
	dev_err(&pdev->dev,"AD:  rproc added\n");
	if (ret)
		goto remove_device_files;
